...
# Keepa prefixes - placeholder objects so prefixes exist
resource "aws_s3_object" "keepa_raw" {
  bucket  = aws_s3_bucket.data.id
  key     = "keepa/raw/.keep"
  content = ""
}
resource "aws_s3_object" "keepa_curated" {
  bucket  = aws_s3_bucket.data.id
  key     = "keepa/curated/.keep"
  content = ""
}
resource "aws_s3_object" "keepa_mart" {
  bucket  = aws_s3_bucket.data.id
  key     = "keepa/mart/.keep"
  content = ""
}
